...
func (_static *CompanionStruct_Default___) ValidRewindPlan(plan RewindPlan) bool {
	var _source0 Option = (plan).Dtor_resetAllChainsTo()
	_ = _source0
	{
		if _source0.Is_None() {
			return ((plan).Dtor_rewindAtOrAfter()).Cmp(Companion_Default___.ACTIVATION__TIMESTAMP()) <= 0
		}
	}
	{
		var _0_ts _dafny.Int = _source0.Get_().(Option_Some).Value.(_dafny.Int)
		_ = _0_ts
		return (((Companion_Default___.ACTIVATION__TIMESTAMP()).Cmp((plan).Dtor_rewindAtOrAfter()) < 0) && ((_0_ts).Cmp(((plan).Dtor_rewindAtOrAfter()).Minus(_dafny.One)) == 0)) && ((((plan).Dtor_targetHeads()).Keys()).Equals(Companion_Default___.CHAIN__IDS()))
	}
}
func (_static *CompanionStruct_Default___) ValidPendingTransition(pending PendingTransition) bool {
	return (((((((!((pending).Dtor_decision()).Equals(Companion_Decision_.Create_Wait_())) && (!(((pending).Dtor_decision()).Equals(Companion_Decision_.Create_Rewind_())) || (((pending).Dtor_rewind()).Is_Some()))) && (!(((pending).Dtor_decision()).Equals(Companion_Decision_.Create_Rewind_())) || (Companion_Default___.ValidRewindPlan(((pending).Dtor_rewind()).Dtor_value().(RewindPlan))))) && (!(((pending).Dtor_decision()).Equals(Companion_Decision_.Create_Advance_())) || (((pending).Dtor_result()).Is_Some()))) && (!(((pending).Dtor_decision()).Equals(Companion_Decision_.Create_Advance_())) || ((((((pending).Dtor_result()).Dtor_value().(Result)).Dtor_invalidHeads()).Cardinality()).Sign() == 0))) && (!(((pending).Dtor_decision()).Equals(Companion_Decision_.Create_Invalidate_())) || (((pending).Dtor_result()).Is_Some()))) && (!(((pending).Dtor_decision()).Equals(Companion_Decision_.Create_Invalidate_())) || ((((((pending).Dtor_result()).Dtor_value().(Result)).Dtor_invalidHeads()).Cardinality()).Sign() == 1))) && (!(((pending).Dtor_result()).Is_Some()) || ((((((pending).Dtor_result()).Dtor_value().(Result)).Dtor_l2Heads()).Keys()).Equals(Companion_Default___.CHAIN__IDS())))
}
func (_static *CompanionStruct_Default___) ValidStepOutput(output StepOutput, obs RoundObservation) bool {
	var _source0 StepOutput = output
	_ = _source0
	{
	...
	}
	{
		if _source0.Is_RewindOutput() {
			return ((obs).Dtor_lastVerifiedTS()).Is_Some()
		}
	}
	{
		if _source0.Is_AdvanceOutput() {
			var _0_result Result = _source0.Get_().(StepOutput_AdvanceOutput).Result
			_ = _0_result
			return (((((_0_result).Dtor_timestamp()).Cmp((obs).Dtor_nextTimestamp()) == 0) && (((_0_result).Dtor_l2Heads()).Equals((obs).Dtor_blocksAtTS()))) && ((((_0_result).Dtor_l2Heads()).Keys()).Equals(Companion_Default___.CHAIN__IDS()))) && ((((_0_result).Dtor_invalidHeads()).Cardinality()).Sign() == 0)
		}
	}
	{
		var _1_result Result = _source0.Get_().(StepOutput_InvalidateOutput).Result
		_ = _1_result
		return (((((_1_result).Dtor_timestamp()).Cmp((obs).Dtor_nextTimestamp()) == 0) && (((_1_result).Dtor_l2Heads()).Equals((obs).Dtor_blocksAtTS()))) && ((((_1_result).Dtor_l2Heads()).Keys()).Equals(Companion_Default___.CHAIN__IDS()))) && ((((_1_result).Dtor_invalidHeads()).Cardinality()).Sign() == 1)
	}
}
func (_static *CompanionStruct_Default___) ValidRoundObservation(obs RoundObservation) bool {
	return ((!(!((obs).Dtor_l1Consistent())) || (((obs).Dtor_lastVerifiedTS()).Is_Some())) && (!((obs).Dtor_chainsReady()) || ((((obs).Dtor_blocksAtTS()).Keys()).Equals(Companion_Default___.CHAIN__IDS())))) && (!((((obs).Dtor_blocksAtTS()).Cardinality()).Sign() == 1) || ((obs).Dtor_chainsReady()))
}
